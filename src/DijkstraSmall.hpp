  ...
  if ( min_distance >= 0.0 )   
	...
	//	 outfile.open("activation.txt",std::ios_base::app);
		//unsigned old_global_index=pNode->mIndex();
               //std::cout << std::to_string(mMapNewOldIndex.at(old_global_index)) << " " << std::to_string(upstroke_time) << std::endl;
...
//		data.push_back(upstroke_time);
            ...
            if( upstroke_time >= mStimuli.size() )
            {
                ...
                {
                    boost::shared_ptr<RegularStimulus> p_stim(new RegularStimulus(-130000.0, 1.0, mperiod,(double)t)); //changed the stim current to -2000000 when moving from ARCHER to MareNostrum
