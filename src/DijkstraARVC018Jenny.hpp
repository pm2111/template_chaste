...
//        int region = mregions[old_index][2];
	...
	//std:nt region = mregions[old_index][2];:cout << "I start to compute the min distances for node " << str << std::endl;
  	        int region = mregions[old_index][2];

	/*Cell Assignment*/
	double distance_epi = p_heart_geom_info->rGetDistanceMapEpicardium()[node_index];
 	double distance_endo;
	if (region!= 1)
	{
        	distance_endo = std::min(lv_dist, rv_dist);
	}
	else
	{
		distance_endo = lv_dist;
