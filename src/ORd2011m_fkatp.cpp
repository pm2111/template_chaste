...
#include "MathsCustomFunctions.hpp"
    ...
    boost::shared_ptr<RegularStimulus> CellORd2011m_fkatpFromCellML::UseCellMLDefaultStimulus()
    {
    ...
    double CellORd2011m_fkatpFromCellML::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[9];
    }
    
    CellORd2011m_fkatpFromCellML::CellORd2011m_fkatpFromCellML(boost::shared_ptr<AbstractIvpOdeSolver> pSolver, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
        : AbstractCardiacCell(
                pSolver,
                41,
                0,
                pIntracellularStimulus)
    {
        ...
        // 
        this->mpSystemInfo = OdeSystemInformation<CellORd2011m_fkatpFromCellML>::Instance();
        Init();
    ...
    CellORd2011m_fkatpFromCellML::~CellORd2011m_fkatpFromCellML()
    {
    }
    
    double CellORd2011m_fkatpFromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_IKr__IKr = (mParameters[10] * 0.80000000000000004) * sqrt(mParameters[2] * 0.18518518518518517) * ((var_IKr__Axrf * var_chaste_interface__IKr__xrf) + ((1.0 - var_IKr__Axrf) * var_chaste_interface__IKr__xrs)) * (((1.0 / (1.0 + exp((var_chaste_interface__membrane__v + 55.0) * 0.013333333333333334))) * 1.0) / (1.0 + exp((var_chaste_interface__membrane__v - 10.0) * 0.033333333333333333))) * (var_chaste_interface__membrane__v - var_reversal_potentials__EK); // microA_per_microF
    ...
    void CellORd2011m_fkatpFromCellML::EvaluateYDerivatives(double var_chaste_interface__environment__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__v = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
    ...
    std::vector<double> CellORd2011m_fkatpFromCellML::ComputeDerivedQuantities(double var_chaste_interface__environment__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__v = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
...
template<>
void OdeSystemInformation<CellORd2011m_fkatpFromCellML>::Initialise(void)
{
