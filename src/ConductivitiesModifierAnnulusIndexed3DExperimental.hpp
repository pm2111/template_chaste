...
#define CONDUCTIVITIESMODIFIERANNULUSINDEXED3D_HPP_
...
class ConductivitiesModifierAnnulus3D : public AbstractConductivityModifier<3,3>
{
private:
   ...
   // const std::vector<double>& mrDistMapRV;
    ...
    std::vector<std::vector<double> >& mregions; 
...
public:
    ...
    ConductivitiesModifierAnnulus3D(
                             const DistributedTetrahedralMesh<3,3>& rMesh,      std::vector<std::vector<double> >& regions,std::vector<unsigned>& perm_vec)
                           : AbstractConductivityModifier<3,3>(),
    mrMesh(rMesh),
