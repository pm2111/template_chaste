...
c_matrix<double,2,2>& Cond2::rCalculateModifiedConductivityTensor(unsigned elementIndex,
                                                           ...
                                                           unsigned domainIndex)
{
	...
	double conductivity_scaling_factor=4.0;
	...
	bool   fibrosis = true;
    	Element<2, 2>* p_element = mrMesh.GetElement(elementIndex);
...
//			Nodo = mrMesh.GetNode( p_element->GetNodeGlobalIndex(i) );
			...
			// Check if new_index exists in endo vector
		
			fibrosis=std::find(mendo.begin(), mendo.end(), MapNewOldIndex.at(new_index)) != mendo.end();

			
//			p_element->ResetIndex(elementIndex);	
...
//			double z = Nodo->rGetLocation()[2];
			...
			std::cout << "Converted node" << std::to_string( new_index) << std::endl;
			i++;
		   }
		//catch (...) {
			//i++;
