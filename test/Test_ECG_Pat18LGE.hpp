...
public:
	...
	{
	  std::cout << "lv  _i basal_anterior_root not specified in commandline args" <<std::endl;
	  exit(1);
	}
...
oost::shared_ptr<SingleTraceOutputModifier> trace_modifier_LL(new SingleTraceOutputModifier("trace_LL.txt", mesh.rGetNodePermutation()[9463801u]));
	...
	*/
	...
	ConductivitiesModifierAnnulus3Dnew conduction_modifier(mesh,regions,perm_vec);
        ...
        std::cout <<" i ve set the mesh" << std::endl;
	
        bidomain_problem.SetWriteInfo();
	        std::cout <<" i ve specified write info" << std::endl;

        bidomain_problem.SetUseHdf5DataWriterCache(true);

        bidomain_problem.Initialise();
        std::cout <<" i ve initialised the prob" << std::endl;
