...
//	#include "boost.hpp"	
	...
	public:
	    ...
	    void TestBoostDijkstra() throw(Exception)
	    {
	        ...
	        std::ifstream edge_file(patient_dir+"lv.edges", std::ios::in|std::ios::binary);
	        char header[36];
	        edge_file.getline(header,36);
	        unsigned num_edges = atoi(header);
	        std::cout<<"Number of edges="<<num_edges<<".\n";
	        std::ifstream weights_file(patient_dir+"lv.weights", std::ios::in|std::ios::binary);
...
126	        int num_arcs = sizeof(edge_array) / sizeof(Edge);*/
	        ...
	        FILE * min_dist_file = fopen((save_dir+"lv.mindists").c_str(), "w");
