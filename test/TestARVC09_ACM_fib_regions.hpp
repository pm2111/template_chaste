...
#include "DijkstraARVC018_ToRORd_ACM_fib_regions.hpp"
...
public:
	...
	else
	{
  	...
  	bool apd_dummy = CommandLineArguments::Instance()->OptionExists("--APD_prol");
	int APD_prol;
        if (apd_dummy == true) {
	
            char* val3 = CommandLineArguments::Instance()->GetValueCorrespondingToOption("--APD_prol");
	    APD_prol=std::atoi(val3);
        }
	else
	{
	  std::cout << "apd prolongation variable not specified in commandline args" <<std::endl;
	  exit(1);
	}
	...
	for (int i =4; i<8;i++)
{
	...
	std::cout << "i get to cell factory" <<std::endl;
        DijkstraStimuliCellFactory cell_factory(&gks_varier,
                                                ...
                                                lv_root_nodes,
                                                rv_root_nodes,
                                                0.150,period,num_nodes,regions,MapNewOldIndex,edge_nodes_path,mindists_path,APD_prol); // 150 cm/s
