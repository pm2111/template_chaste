...
public:
	...
	else
	{
	  ...
	  exit(1);
       ...
       bool root7 = CommandLineArguments::Instance()->OptionExists("--rv_mid_anterolateral_root2");
        if (root7 == true) {
            char* val = CommandLineArguments::Instance()->GetValueCorrespondingToOption("--rv_mid_anterolateral_root2");
            dummy = std::atoi(val);
	    nodes_present.push_back(dummy);
        }
	else
	{
	  std::cout << "rv_mid_anterolateral_root2 not specified in commandline args" <<std::endl;
	  exit(1);
	}
